...
export default function MangaReader({ images }: MangaReaderProps) {
  ...
  const sortedImages = [...images].sort((a, b) => a.imageNumber - b.imageNumber);
  ...
  const handleImageClick = (e: React.MouseEvent<HTMLImageElement>) => {
    ...
    clickPosition < width / 2 ? swiperRef.current.slidePrev() : swiperRef.current.slideNext();
  };
  ...
  return (
    <div className="w-full p-4 bg-black text-white min-h-screen">
      <div className="flex justify-between items-center mb-4">
        ...
        {/* View Mode Dropdown */}
        <select
          className="px-3 py-1 bg-[#808080] text-white text-sm cursor-pointer"
      ...
      {viewMode === "full" ? (
        <div className="flex flex-col">
          {sortedImages.map((img) => (
            <img key={img.id} src={img.image} alt={`Manga Page ${img.imageNumber}`} className="w-full" loading="lazy" />
          ))}
        </div>
      ) : (
        <Swiper
        ...
        >
          {sortedImages.map((img) => (
            <SwiperSlide key={img.id}>
              <img src={img.image} alt={`Manga Page ${img.imageNumber}`} className="w-full cursor-pointer" onClick={handleImageClick} />
            </SwiperSlide>
          ))}
        </Swiper>
      )}
