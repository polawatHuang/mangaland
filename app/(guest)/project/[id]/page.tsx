    ...
    <div className="relative w-full min-h-screen max-w-6xl mx-auto md:p-8 pb-20 gap-16 sm:p-2">
      ...
      <section className="md:px-[12%]">
        <div className="w-full bg-[#1f2936] px-4 py-2">
          <Link href="/">Homepage</Link> / <Link href={`/project/${params.id}`}>{manga.name}</Link>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-4">
            {manga.episodes.length > 0 ? (
              manga.episodes.map(({ episodeNumber, title, createdAt }) => (
                <Link key={episodeNumber} href={`/project/${params.id}/ep${episodeNumber}`} className="bg-blue-500 hover:bg-blue-600 px-4 py-2 flex justify-between">
                  <span className="text-white font-[600]">ตอนที่ {episodeNumber} - {title}</span>
                  <span className="text-white opacity-50">{dayjs(createdAt).format("DD/MM/YYYY")}</span>
                </Link>
              ))
            ) : (
              <p className="text-gray-500">ขออภัยค่ะ ไม่มีตอนที่ระบุดังกล่าว</p>
            )}
          </div>
        </div>
      </section>
